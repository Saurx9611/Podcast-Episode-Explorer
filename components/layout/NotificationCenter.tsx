...
const FALLBACK_NOTIFICATIONS: NotificationItem[] = [
  {
    ...
    timestamp: '2 min ago',
    read: false,
    link: '/episodes/ep-001'
  },
  {
    id: '2',
    type: 'processing_failed',
    title: 'Episode processing failed',
    description: '"Corrupted Audio File" could not be processed.',
  ...
  },
  {
    ...
    description: '"Database Bottlenecks" returned new relevant segments.',
  ...
  },
  {
    ...
    description: '"React Server Components" has been added to semantic search.',
    timestamp: '3 hours ago',
    read: true,
    link: '/episodes/ep-002'
  },
  {
    id: '5',
    type: 'processing_started',
    title: 'Processing started',
    description: '"Machine Learning Ops" is being transcribed.',
...
export function NotificationCenter() {
  ...
  const handleNotificationClick = async (id: string, link?: string) => {
    try {
      ...
      console.warn('Could not mark notification as read on backend:', err);
    }
  ...
  const getIcon = (type: NotificationType) => {
    switch (type) {
      ...
      case 'saved_search_result':
        return <Search className="w-4 h-4 text-[var(--color-accent)]" />;
      case 'transcript_indexed':
        return <FileText className="w-4 h-4 text-purple-400" />;
      default:
        return <Info className="w-4 h-4 text-[var(--color-secondary)]" />;
    }
  };
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-border)] shrink-0">
            <div className="flex items-center gap-2">
              ...
              {loading && <Loader2 className="w-3.5 h-3.5 animate-spin text-[var(--color-secondary)]" />}
            </div>
            {unreadCount > 0 && (
              <button 
                ...
                {filteredNotifications.map((notification) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      ...
                      {(notification.type === 'processing_failed' || notification.type === 'saved_search' || notification.type === 'saved_search_result') && (
                        <div className="pt-1">
